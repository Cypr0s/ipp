...
docker_build() {
    ...
    if [[ "${BUILD_OUTPUT:-0}" == "1" ]]; then
        (cd "${ARCHIVE_ROOT}" && timeout 600 "${CONTAINER_ENGINE}" build "${file_arg[@]}" "$@" .) 2>&1 | tee "${log_file}"
        rc=${PIPESTATUS[0]}
    else
        (cd "${ARCHIVE_ROOT}" && timeout 600 "${CONTAINER_ENGINE}" build "${file_arg[@]}" "$@" .) >"${log_file}" 2>&1
        rc=$?
    fi
...
build_and_test_build() {
    ...
    if [[ "${need_build}" == "true" ]]; then
        ...
        if (cd "${ARCHIVE_ROOT}" && timeout 600 "${CONTAINER_ENGINE}" build "${file_arg[@]}" --target build --progress plain .) 2>"${log}"; then
    ...
    if [[ "${need_build_test}" == "true" ]]; then
        ...
        if (cd "${ARCHIVE_ROOT}" && timeout 600 "${CONTAINER_ENGINE}" build "${file_arg[@]}" --target build-test --progress plain .) 2>"${log}"; then
