...
class SolClass:
    ...
    """
    def __init__(self, name: str, super_class: SolClass | None, methods: dict[str, SolMethod]) -> None:
        self.name: str = name
...
class SolMethod:
    def __init__(self, selector: str, is_builtin: bool, func: "callable | Block", arity: int) -> None:
...
class Scope:
    ...
    def get_object(self, object_name: str) -> SolObject | None:
        ...
        raise InterpreterError(
                error_code=ErrorCode.SEM_UNDEF, 
                message=f"No object with `{object_name}` exists"
            )
